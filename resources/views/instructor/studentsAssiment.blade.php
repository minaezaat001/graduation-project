...
<body>
    ...
    <!-- Start NavBar -->
    <header>
        <img class="w-100" src="images/header/wave.svg" alt="" draggable="false" />
        <img src="images/logo/logo.jpg" alt="" class="logo" width="100px" />
    </header>
    <div class="menu-tab">
        <div id="one"></div>
        <div id="two"></div>
    ...
    <section id="lectures" class="test block">
        <div class="container">
            <div class="card text-left">
                <div class="card-body">
                    <h4 class="card-title">الاسم : {{ $researches->Name }}</h4>
                    <p class="card-text">عدد الابحاث : {{ $researches->attachmentassiment->count() }}</p>
                </div>
            </div>
            <div class="py-4">
                    ...
                    <table class="table table-hover">
                        <thead class="thead-colorful">
                            <tr>
                                ...
                                <th scope="col">عرض البحث</th>

                            </tr>
                        </thead>
                        <tbody>

                            @forelse ($researches->attachmentassiment as $researchess)
                                <tr>
                                    <th scope="row">{{ $researchess->id }}</th>
                                    <td>{{ $researchess->student->Name }}</td>
                                    <td>{{ $researchess->student->grade->Name }}</td>
                                    <td><a href="{{ URL::asset($researchess->attach) }}" target="_blank">عرض</td>
                                </tr>
                            @empty
                            @endforelse
                        </tbody>
